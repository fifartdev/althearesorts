...
export default function GreekJournalPage() {
  const featured = JOURNAL_POSTS_EL[0]
  const gridPosts = JOURNAL_POSTS_EL.slice(1)
  ...
  return (
    <main id="main-content">
      ...
      <section className="pb-16 bg-white">
        <div className="container-luxury">
          <ScrollReveal>
            <Link href={`/el/journal/${featured.slug}`} className="group grid grid-cols-1 lg:grid-cols-2 gap-8 border border-[#e8e4dd] overflow-hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {gridPosts.map((post, i) => (
              <ScrollReveal key={post.slug} delay={i * 60}>
                <Link href={`/el/journal/${post.slug}`} className="group block">
